...
<div class="inner-banner inner-bg2">
    <img width="1920" height="400" data-src="<?= !empty($product->banner_url) ? get_picture("products_v", $product->banner_url) : get_picture("settings_v", $settings->product_detail_logo) ?>" class="img-fluid w-100 lazyload" alt="<?= strto("lower|upper", $product->title) ?>">
    <div class="container">
        <div class="inner-title text-center">
            <h3><?= strto("lower|upper", $product->title) ?></h3>
            <ul>
                ...
                <?php if (!empty($product->category_ids)) : ?>
                    <li>
                        ...
                        <?php foreach (explode(",", $product->category_titles) as $k => $v) : ?>
                            ...
                            <?php if ($i < $count) : ?>
                                <a rel="dofollow" href="<?= base_url(lang("routes_products") . "/{$seo_url}" . (!empty($_GET["key"]) ? "?key=" . clean($_GET["key"]) : null)) ?>" title="<?= strto("lower|upper", $v) ?>"><?= strto("lower|upper", $v) ?></a>,
                            <?php else : ?>
                                <a rel="dofollow" href="<?= base_url(lang("routes_products") . "/{$seo_url}" . (!empty($_GET["key"]) ? "?key=" . clean($_GET["key"]) : null)) ?>" title="<?= strto("lower|upper", $v) ?>"><?= strto("lower|upper", $v) ?></a>
                            <?php endif ?>
                            <?php $i++ ?>
                    ...
                    <li><i class="fa fa-angle-right"></i></li>
                <?php endif ?>
                <li><?= strto("lower|upper", $product->title) ?></li>
            </ul>
        </div>
    </div>
</div>
...
<div class="product-details-area pt-100 pb-70">
    <div class="container">
        <div class="row align-items-top">
                            ...
                            <div class="col-9 col-sm-9 col-lg-12 col-xl-12 col-xxl-10 order-2 order-lg-1 order-xxl-2">
                                <div class="carousel-inner">
                                    ...
                                    <?php if (!empty($product_own_images)) : ?>
                                        <?php foreach ($product_own_images as $k => $v) : ?>
                                            <?php if ($v->product_id == $product->id) : ?>
                                                <div class="carousel-item article-image item <?= $i == 0 ? "active" : null ?>" data-index="<?= $i ?>">
            ...
            <div class="col-lg-12 justify-content-center text-start-xl justify-content-xl-start">
                <div class="product-desc">
                    <div class="article-content mt-0">
                        <h2><?= $product->title ?></h2>
                        <ul class="reviews ms-0 ps-0">
                            <?php $i = 1 ?>
                            <?php $count = count(explode(",", $product->category_ids)) ?>
                            <?php foreach (explode(",", $product->category_titles) as $k => $v) : ?>
                                <?php $seo_url = explode(",", $product->category_seos)[$k]; ?>
                                <li class="d-inline-flex w-auto">
                                    <?php if ($i < $count) : ?>
                                        <a rel="dofollow" href="<?= base_url(lang("routes_products") . "/{$seo_url}" . (!empty($_GET["key"]) ? "?key=" . clean($_GET["key"]) : null)) ?>" title="<?= $v ?>"><?= $v ?></a>,
                                    <?php else : ?>
                                        <a rel="dofollow" href="<?= base_url(lang("routes_products") . "/{$seo_url}" . (!empty($_GET["key"]) ? "?key=" . clean($_GET["key"]) : null)) ?>" title="<?= $v ?>"><?= $v ?></a>
                                    <?php endif ?>
                                </li>
                        ...
                        <?= $product->description ?>
                        <ul class="tag stockWrapper d-none">
                            <li><?= lang("stockStatus") ?>:
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12">
                                <ul class="tabs">
                                    <?php if (!empty(clean($product->features))) : ?>
                                        <li>
                                            <a rel="dofollow" href="<?= base_url() ?>" title="<?= lang("features") ?>">
                                                <?= lang("features") ?>
                                            </a>
                                        </li>
                                    <?php endif ?>
                                    <?php if (!empty(clean($product->content))) : ?>
                                        <li>
                                            <a rel="dofollow" href="<?= base_url() ?>" title="<?= lang("description") ?>">
                                                <?= lang("description") ?>
                                            </a>
                                        </li>
                                    <?php endif ?>
                            ...
                            <div class="col-lg-12 col-md-12">
                                <div class="tab_content current active pt-45">
                                    ...
                                    <?php endif ?>
                                    <?php if (!empty(clean($product->content))) : ?>
                                        <div class="tabs_item <?= (empty(clean($product->features)) ? "current" : null) ?>">
                                            <div class="products-tabs-decs">
                                                <?= $product->content ?>
                                            </div>
                                        </div>
